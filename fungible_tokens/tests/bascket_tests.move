...
module fungible_tokens::basket_tests {
    ...
    public fun test_mint_burn() {
        ...
        test_scenario::next_tx(scenario, user);
        {
            ...
            let bascket = basket::mint(reserve, sui, managed, ctx);
            assert!(coin::value(&bascket) == num_coins, 1);
            assert!(basket::total_supply(reserve) == num_coins, 2);


            let (sui, managed) = basket::burn(reserve, bascket, ctx);
            assert!(coin::value(&sui) == num_coins, 3);
            assert!(coin::value(&managed) == num_coins, 3);
